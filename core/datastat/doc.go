...
// Go 重锚定，PyR 波次）：CTT 经典测量理论、IRT 2PL 边际最大似然（MML）估计、
// 先验/实测贝叶斯收缩、掌握度 Elo、五轴覆盖缺口、题目健康度与生命周期状态机、
// 增量重判/全量重放聚合核。
//
...
//   - IRT 2PL 标定落库（irt 的 item_param 写入面）——source=measured_irt/
//     method_version=irt-2pl-v1（item_param CHECK 已容纳，无 schema 变更）；
//     合成作答标定走独立 source（measured_sim_irt）严格分账；CLI 见 cmd/calibrate；
