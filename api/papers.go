...
func createPaper(papers PapersWiring) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			// 日志只落 route pattern + 哨兵分型常量：错误值可能携带请求方
			// 可控字节（蓝图解析链），errClass(err) 的 %T 派生同样被视为
			// 注入面（CodeQL go/log-injection #34/#35 的修法）——分型用
			// errors.Is 归一到固定短码，不回显错误派生值.
			class := paperFailureClass(err)
			log.Printf("paper orchestration failure route=%s error_class=%s", r.Pattern, class)
...
func generateDraftItems(papers PapersWiring) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Printf("generate failure route=%s error_class=%s", r.Pattern, generateFailureClass(err))
			writeErrorClass(w, http.StatusUnprocessableEntity, middleware.ErrorClassBadRequest)
			return
		}
		w.Header().Set("Content-Type", "application/json")
...
// paperFailureClass 把编排失败归一到固定短码（日志安全：只输出常量，
// 不回显错误派生值——go/log-injection #34 的修法）.
func paperFailureClass(err error) string {
	var infeasible *assembly.InfeasibleError
	switch {
	case errors.As(err, &infeasible):
		return "infeasible"
	case errors.Is(err, assembly.ErrInvalidCandidateRow):
...
// generateFailureClass：subjectmath.Run 的错误为非哨兵 fmt.Errorf（可能
// 内嵌请求方 template_id），日志分型一律固定常量，不回显错误派生值.
func generateFailureClass(_ error) string {
