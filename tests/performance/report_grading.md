...
> 生成时间：2026-08-29 16:08:02（由 test_grading_latency.py 刷新）
...
- 样本数：100
- 平均延迟：1.56 ms（阈值 10000 ms）
- **p95：2.50 ms**（阈值 15000 ms）
- 平均判定：✅ 通过
- p95 判定：✅ 通过
...
- 评分器计算（run_scorer，无 DB）平均：0.042 ms
- DB 写入 + 落账编排平均：1.522 ms
- 总链路平均：1.564 ms
- 占比：评分器 2.7% / DB+编排 97.3%

### 按交互类型拆分

| 交互类型 | 样本数 | 平均 (ms) | p95 (ms) |
|---|---|---|---|
| single_choice | 34 | 1.628 | 2.371 |
| numeric_blank | 33 | 1.538 | 2.571 |
| matching | 33 | 1.524 | 2.179 |
