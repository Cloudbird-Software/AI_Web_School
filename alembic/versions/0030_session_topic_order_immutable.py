"""T-W5-004 会话题序不可变与结构性 DB 约束（W5-R Go 重锚定，镜像 db/migrations/0030_*.sql）.

缺陷事实（任务卡目标说明 + 0011/0019 现状实读）：
- practice_session.item_sequence（题序 + placement_token）是 A4 追溯链路的锚点，
  0011 仅以 NOT NULL + 「应用层只写一次」纪律保证创建后不变——任意 UPDATE 仍可
  改写题序，改写后历史作答（response_event）无法与题目对应；DELETE 整行更可直接
  销毁锚点（response_event 无 FK 指回会话，0003 现状）。
...
- A1. practice_session 题序/身份锚列（session_id/student_alias_id/scene/
  item_sequence）创建后拒绝 UPDATE——条件式行级触发器，运行态列（current_index/
  status/answered_count/correct_count/wrong_marks/last_*/completed_at 等）明确
  放行：0011 docstring「会话进度是运行态操作数据，必须随作答推进原地更新」，
  任务卡验收 #1 的字面语义；与 0024 内容四表整表触发器的关键差异即在此（本表
  不在 D1 三本账内，E2E-6 的会话状态同事务推进依赖运行态列可更新）。
- A2. 整行 DELETE 拒绝（BEFORE DELETE 行级触发器，复用 0005 函数原句）——无
  任何合法删会话路径（冻结实现 abandon 是 status 更新非删行），删行 = 销毁 A4
  题序锚点。
...
- B2. 题序行结构 + (session_id, seq) 唯一（BEFORE INSERT 行级触发器完整校验：
  ...
  placement_token 文本或 null、item_number 数组内唯一——重复即以 ERRCODE 23505
  + 约束名 uq_session_topic_order_seq 显式暴露，应用层 core/session 按唯一冲突
  映射哨兵错误）。UPDATE 面结构校验结构性不可达：item_sequence 任何改写已被
  A1 无条件拒绝。
- C1. spec_table cells 容器 CHECK（非空数组；0019 仅校验「是数组」）。
- C2. spec_table cells 结构触发器（BEFORE INSERT 完整校验，本表 append-only 故
  INSERT 即唯一写面）：条目必为对象、必需键 content_code/cognitive_level/
  target_count/difficulty_min/difficulty_max、target_count 为非负整数、难度区间
  0≤min≤max≤1（p_correct 口径）、cognitive_level 六值域、(content_code,
  cognitive_level) 身份唯一（单元格数与声明一致——
  src/core/assembly/spec_table.py SpecTable._check_cell_uniqueness 的结构性
  表达）、Σtarget_count > 0（_check_total_count_positive 同源）。全部为冻结
  SpecTable/SpecCell 构造期不变式的 DB 级固化，非新语义。

与 migrate-go-check 探针的关系（如实声明）：practice_session/spec_table 均不做
整表 append-only（运行态更新/版本账 INSERT 是合法写面），不落入 migrate_check
第 3 段「BEFORE UPDATE OR DELETE ON <表>」自动探针清单；本迁移的行为验收（题序
UPDATE 拒 / 运行态 UPDATE 放 / DELETE 拒 / 非法结构拒）由 CI 全量 cycle（第 2 段
down→up 可逆性）+ Go 侧 core/session 语义测试承担。

可逆性（SQL-1）：downgrade 精确 DROP 本迁移新建的触发器/函数/约束（IF EXISTS
成对语义）；不触碰 0005 raise_append_only_error / 0019
raise_spec_table_append_only_error 等既有函数体（禁 CREATE OR REPLACE 重定义，
T-W5-032 #43 裁决）。

链序说明：down_revision 指 0029（main 现有最新）；编号取 0030（main 迁移面
0001..0029 连续，migratechain 守卫不许跳号——0029_pii_vault_roles 合入时已按
同守卫把并行分支的 0030 改号为 0029，本迁移顺延取 0030，无并行分支占用）。
...
# 语句文本与 db/migrations/0030_session_topic_order_immutable.up.sql 逐句一致
# （migrate-go-check parity 按 pg_dump 归一化语句多重集比较）。

_CREATE_ANCHOR_GUARD_FN = """
...
_CREATE_SEQUENCE_STRUCTURE_TRIGGER = """
...
_CREATE_CELLS_VALIDATOR_TRIGGER = """
...
def upgrade() -> None:
    """会话题序锚不可变 + 题序/细目表 cells 结构性 DB 约束（全加性）."""
    for sql in _UPGRADE_STATEMENTS:
        op.execute(sql)


def downgrade() -> None:
    """成对回滚：精确 DROP 本迁移新建的触发器/函数/约束."""
    for sql in _DOWNGRADE_STATEMENTS:
        op.execute(sql)
